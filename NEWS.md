...
A crosswalk from facilities to statistical regions is built once and read
many times, so an error in it is the one error recomputing the downstream
tables cannot find: everything downstream reads the crosswalk, and both
sides of any comparison move together. `R/crosswalk.R` adds the checks
that can find it.
