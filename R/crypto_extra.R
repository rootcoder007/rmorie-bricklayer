...
#' can be recovered from its output; a key drawn from it is guessable.
#' Reading the OS source also leaves R's own random stream untouched, so
#' generating a key does not perturb a reproducible analysis.
#'
#' If no OS source can be read the function FAILS rather than falling
#' back to a weaker generator, because a silent downgrade in a key is
#' worse than an error.
#'
#' @param n Number of bytes (1 to 1048576).
#' @return A raw vector of length `n`.
#' @seealso [derive_key()] to stretch a passphrase instead,
...
#'
#' PBKDF2-HMAC-SHA256 (RFC 8018): stretches a passphrase into a key of
#' full width by iterating a keyed hash, so guessing the passphrase costs
#' `iterations` times more than a single hash would.
#'
#' The `salt` must be UNIQUE per key and need not be secret. Its job is
#' to make precomputation useless: without one, a single table of common
#' passphrases attacks every key at once.
#'
#' `iterations` is the cost knob. The default 100,000 is a reasonable
#' 2020s floor for an interactive use; raise it for anything valuable,
#' and record the value you used, since verification must repeat it
#' exactly.
#'
...
#'
#' @param passphrase Passphrase, as a length-1 character or raw vector.
#' @param salt Unique, non-secret salt (length-1 character or raw). Use
#'   [random_bytes()] to make one, and store it beside the key.
#' @param iterations Iteration count (default 100000, minimum 1).
#' @param length Derived key length in bytes (default 32).
#' @return A length-1 character vector: the key as lowercase hex.
#' @references Moriarty K, Kaliski B, Rusch A (2017). PKCS #5: Password-Based
#'   Cryptography Specification Version 2.1. RFC 8018.
#'   \doi{10.17487/RFC8018}
#' @seealso [random_bytes()], [core_hmac_sha256()], [capsule_sign()]
#' @examples
...
#'
#' BLAKE2b (RFC 7693): a modern cryptographic hash, faster than SHA-256
#' in software, that takes a key natively and produces any digest length
#' from 1 to 64 bytes.
#'
...
#' [core_hmac_sha256()] at lower cost. Use SHA-256 or HMAC where
#' interoperability with other tools matters; use this where it does not
#' and speed does.
#'
#' @param x A character vector or a raw vector.
#' @param key Optional key, at most 64 bytes. `NULL` (default) gives the
#'   plain digest.
#' @param length Digest length in bytes, 1 to 64 (default 32, matching
#'   SHA-256's width).
...
#' @seealso [core_sha256()], [core_sha512()], [core_hmac_sha256()]
#' @examples
...
#' hashes.
#'
#' Serialization is pinned to version 2 with XDR byte order, so the
#' digest is the same on a big-endian machine as on a little-endian one.
#' Two objects that are `identical()` produce the same digest; two that
#' merely print the same need not, because attributes are part of the
#' serialization.
#'
#' @param x Any R object.
#' @param algo `"sha256"` (default), `"sha512"`, `"blake2b"` or
#'   `"crc32"`.
#' @param key Optional key, for `"blake2b"` only: gives a keyed
#'   fingerprint that only a key holder can reproduce.
#' @return A length-1 character vector (or numeric for `"crc32"`).
#' @seealso [core_sha256()] for hashing text or bytes directly,
#'   [bricklayer_json_serialize()] for a readable lossless form.
#' @examples
