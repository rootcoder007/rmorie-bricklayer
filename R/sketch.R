...
#'
#' An accumulator for the mean, variance, skewness and kurtosis of a
#' column that does not fit in memory. Feed it blocks with
#' [summary_update()] and read the result with [summary_stats()].
#'
#' The merge is EXACT, not approximate: it uses Chan, Golub and
#' LeVeque's parallel combination of central sums, extended to the third
#' and fourth moments by Terriberry, so accumulating a column in blocks
#' gives the same answer as [core_moments()] over the whole column at
#' once. That is what makes it usable for a capsule member of any size:
#' memory stays constant in the number of blocks.
#'
#' The object is a plain list and is copied on assignment like any other
#' R value, so `update` returns the new accumulator and you must keep it:
#' `acc <- summary_update(acc, block)`.
#'
#' @param x Numeric vector: the first block, or `NULL` for an empty
#'   accumulator.
#' @param acc,a,b Accumulators from `online_summary()`.
#' @return `online_summary()`, `summary_update()` and `summary_merge()`
#'   return an object of class `bricklayer_online`; `summary_stats()`
#'   returns a named numeric with `n`, `mean`, `variance`, `sd`,
...
#' @seealso [core_moments()] for the single-pass batch version.
#' @examples
#' set.seed(1)
#' x <- stats::rnorm(1000)
#'
...
#' likely to be retained, in a single pass and using memory proportional
#' to `k` rather than to the stream's length. Use it to take a fair
#' sample of a capsule member you cannot hold, or whose length you do not
#' know in advance.
#'
...
#' @param k Sample size. Values above the stream length give the whole
#'   stream.
#' @param seed Seed for the core's generator (default 42).
#' @return `reservoir_indices()` a sorted numeric vector of 1-based
#'   positions; `reservoir_sample()` the corresponding elements of `x`.
...
#'
#' HyperLogLog: estimates how many distinct values a column holds using
#' a fixed 4-byte register per bucket -- 64 KB at the default `p = 14`
#' -- regardless of the column's length or cardinality. Exact counting
#' needs memory proportional to the number of distinct values, which is
#' the thing you cannot afford on a capsule member of unknown size.
#'
...
#'
#' Values are hashed with the package's SHA-256, so the sketch is
#' identical on every platform and across sessions.
#'
...
#' @param registers Registers from a previous call, to fold another chunk
#'   into the same sketch.
